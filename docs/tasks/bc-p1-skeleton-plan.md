...
| 2026-09-06 | M14 Kyverno 生产基线（root 用户 + procMount + hostAlias） | ✅ 完成。在 M13 十一层基础上新增三层 Pod 级策略，形成十四层纵深防御：⑨ `disallow-run-as-root`（runAsUser != 0，作为 PSS runAsNonRoot 的纵深防御补充）、⑩ `require-default-proc-mount`（procMount 必须为 Default，禁止 Unmasked 暴露宿主机 /proc）、⑪ `disallow-host-aliases`（禁止 spec.hostAliases，防 DNS 劫持）。kind 实证：runAsUser=0 被拒、hostAliases 被拒、全合规 Pod 放行+digest 固定。偏差：`procMount: Unmasked` 需 Kubernetes `ProcMountType` 特性门控，kind 集群未启用时 API Server 自动转为 Default，无法实证违规但策略逻辑正确 |
| 2026-09-06 | M15 Kyverno RBAC 最小权限 | ✅ 完成。在 M14 十四层基础上新增三层 RBAC 级策略，形成十七层纵深防御：⑫ `disallow-cluster-admin-binding`（禁止 ClusterRoleBinding/RoleBinding 绑定 cluster-admin）、⑬ `disallow-wildcard-rbac`（禁止 Role/ClusterRole 中 verbs/resources/apiGroups 含 `*`）、⑭ `disallow-default-service-account`（platform ns Pod 禁止用 default SA，须显式指定专用 SA）。kind 实证：cluster-admin 绑定被拒、通配符 verbs 被拒、default SA Pod 被拒；合规非 admin 绑定/显式权限 Role/专用 SA Pod 放行。偏差：RBAC 策略因 Kyverno SA 无 RBAC 资源 get/list/watch 权限，须关闭 background 扫描（`evaluation.background.enabled=false`），仅准入时校验 |
| 2026-09-07 | M16 Kyverno 服务暴露与存储治理 | ✅ 完成。在 M15 十七层基础上新增三层策略，形成二十层纵深防御：⑫ `disallow-node-port-lb-service`（禁止 platform ns NodePort/LoadBalancer Service，对外经 Ingress/网关）、⑬ `require-emptydir-size-limit`（emptyDir 卷必须设 sizeLimit）、⑭ `require-always-pull-policy`（imagePullPolicy 必须 Always，配合签名策略 mutateDigest 转 digest 后自动设为 IfNotPresent）。kind 实证：NodePort Service 被拒、emptyDir 无 sizeLimit 被拒、imagePullPolicy=IfNotPresent 被拒；合规 ClusterIP/有 sizeLimit/Always 放行。偏差 ①：Kyverno chart 3.9.0 默认用 reg.kyverno.io（不可达），values.yaml 覆盖 image.registry=ghcr.io + 手动 patch 全部 deployment 镜像；偏差 ②：emptyDir CEL `.filter(v, v.?emptyDir.orValue({}) != {})` 无法检测空对象，须改用 `.orValue(null) != null` 判断字段存在性 |
| 2026-09-07 | M17 Kyverno 高可用与可观测性 | ✅ 完成。在 M16 二十层基础上新增三层策略，形成二十三层纵深防御：⑮ `require-health-probes`（容器必须配 livenessProbe + readinessProbe）、⑯ `require-deployment-multi-replicas`（Deployment replicas >= 2，关闭 background 扫描）、⑰ `require-ingress-tls`（Ingress 必须配 TLS 段且显式声明 hosts）。kind 实证：无探针 Pod 被拒、replicas=1 Deployment 被拒、无 TLS Ingress 被拒；合规 replicas=2/TLS Ingress 放行。关键发现：ValidatingPolicy 匹配 pods 时 Kyverno autogen 自动覆盖 Deployment 等控制器的 Pod 模板（测试 Deployment 副本策略时 Pod 模板被资源/探针策略先行校验） |
| 2026-09-07 | M18 身份基础设施（Keycloak + APISIX OIDC + JWKS 验签） | ✅ 完成。目标：打通「能认证」链路，为 M19 门户前端提供登录基础（计划文档 .trae/documents/m18_keycloak_oidc_plan.md）。交付：① `deploy/identity/keycloak/`——realm 引导声明 jsl-realm.json（client jsl-gateway、tenant_id 用户属性映射、demo-admin/dev）、Helm values、realm ConfigMap；② `deploy/gateway/apisix/`——standalone 路由（/realms 透传 Keycloak，/api 走 proxy-rewrite + openid-connect bearer 验签 + serverless-pre-function Lua 从 JWT 注入 x-tenant-id）；③ `pkg/authz/jwks.go`——JWKSVerifier（coreos/go-oidc v3.21，RS256 discovery/JWKS，iss/aud/exp/tenant_id 全校验，fail-closed）+ `NewVerifierFromEnv`（JSL_AUTHZ_MODE=stub\|jwks，默认 stub 向后兼容），10 个新单测全绿；④ main.go 接线（tenant-bc）；⑤ compose 栈加入 keycloak/apisix/tenant-bc 三服务；⑥ GitOps：keycloak.yaml、apisix.yaml 多源 ArgoCD Application + namespaces/identity.yaml；⑦ m18-auth-smoke.sh 冒烟脚本。compose 实证 5 用例全过：无令牌 401、合法令牌通过（业务 not_found 证明认证链全通）、篡改签名 401、Keycloak 经网关 200、伪造 x-tenant-id 被网关覆盖。踩坑：① Keycloak realm JSON 不允许 `_comment` 字段（导入直接失败）；② Keycloak 26 访问令牌默认不含 `aud` 声明，需 oidc-audience-mapper，否则 go-oidc 受众校验 401；③ compose healthcheck 须带 Connection: close 否则 cat 挂起超时，且响应 JSON 带空格（`"status": "UP"`）；④ 网关 bearer_only 模式（浏览器 code 流由 M19 BFF 承接），Lua 用 core.request.set_header 覆盖客户端伪造租户头。kind GitOps 清单已就绪待集群验证（chart 版本 keycloak 26.1.* / apisix 2.15.*） |
